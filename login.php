<?php
  ...
  if (isset($_SESSION["username"])) { //Cas ou l'on est connecté
    ...
    if (isset($_POST["username"]) && !empty($_POST["username"]) && isset($_POST["password"]) && !empty($_POST["password"])) {
      $req = "SELECT username, password FROM users WHERE username=\"".safehtml($_POST["username"])."\"";
      $res = mysqli_query($co, $req);
      $x;
      if ($res) {
        $ligne = mysqli_fetch_assoc($res);
      ...
      if (password_verify(safehtml($_POST['password']), $x)) {
        $_SESSION['username'] = safehtml($_POST['username']);
      ...
      if (isset($_POST["username"]) && !empty($_POST["username"])) echo (" value=\"".safehtml($_POST["username"])."\"");
      echo("><br>
      Mot de passe<br>
